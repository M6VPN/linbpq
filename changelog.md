...
- Fixed NET/ROM trace report assembly so L2/L3 trace JSON appends and NET/ROM decoders stop safely on truncation or short packet data. ([218a157])
- Fixed L2 and node-up event reporting so generated UDP JSON is bounded and truncated startup/link reports are not sent. ([1307b2e])
- Fixed circuit up/down event reporting so generated UDP JSON is bounded and truncated transition reports are not sent. ([0d5592e])
- Fixed circuit status event reporting so generated UDP JSON is bounded and truncated status reports are not sent. ([2c3e1e0])
...
- `git diff --check` completed after the NET/ROM trace report assembly fix.
- `make -f makefile -B Events.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` completed with remaining existing unrelated warnings after the NET/ROM trace report assembly fix.
- `git diff --check` completed after the L2 and node-up event reporting fix.
- `make -f makefile -B Events.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` completed with remaining existing unrelated warnings after the L2 and node-up event reporting fix.
- `git diff --check` completed after the circuit up/down event reporting fix.
- `make -f makefile -B Events.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` completed with remaining existing unrelated warnings after the circuit up/down event reporting fix.
- `git diff --check` completed after the circuit status event reporting fix.
- `make -f makefile -B Events.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` completed with remaining existing unrelated warnings after the circuit status event reporting fix; the `L4StatusSeport` format warnings are cleared.
...
- `make -f makefile -B Events.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` completed with remaining existing unrelated warnings after the L4 session MQTT report formatting fix; the `hookL4SessionDeleted` format warning is cleared.
...
- Fixed DRATS frame debug logging so received, decoded, and encoded protocol text is logged with literal format strings. ([05df6bf])
...
- `make -f makefile -B FLDigi64.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` is blocked by existing Linux integration errors in `FLDigi64.c`, including `PortConfig`/`TNCInfo` conflicts, `MAXBPQPORTS` redefinition, `Rig_Command` argument mismatches, and missing `BytesTXed`/`BytesRXed` members.
...
- `git diff --check` completed after the VARA debug format fix.
...
- `git diff --check` completed after the WINMOR debug format fix.
- `make -f makefile -B WINMOR.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` completed with existing unrelated pointer-sign and unused warnings after the WINMOR debug format fix.
- `git diff --check` completed after the PCAP error debug format fix.
- `make -f makefile -B IPCode.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` completed with existing unrelated warnings after the PCAP error debug format fix.
...
- `git diff --check` completed after the DRATS debug format fix.
- `make -f makefile -B DRATS.o CFLAGS='-DLINBPQ -MMD -g -fcommon -fasynchronous-unwind-tables -Wall -Wextra -Wformat -Wformat-security -Wstringop-overflow -Warray-bounds -DNOMQTT'` completed with existing unrelated warnings after the DRATS debug format fix.
